...
`layout/header.php` renders a Bootstrap 4 navbar with links to every module:
...
| Monitores | `pages/monitores.php` |
| Equipos | `pages/equipos.php` |
| Mantenimientos | `pages/mantenimientos.php` |
| Ver Mantenimientos | `pages/ver-mantenimientos.php` |
...
<Columns cols={2}>
  ...
  <Card title="Edit" icon="pencil">
    Clicking an edit button appends an `?edit=ID` query string. PHP detects `$_GET['edit']`, fetches the record, and pre-fills the form fields for the user to modify.
  </Card>
  <Card title="Update" icon="floppy-disk">
    The pre-filled form POSTs back to the page. PHP detects a hidden `id` field in `$_POST` and runs an `UPDATE` query instead of an `INSERT`.
  </Card>
  <Card title="Delete" icon="trash">
    Clicking a delete button appends `?delete=ID`. PHP detects `$_GET['delete']` and runs a `DELETE` query, then redirects.
  </Card>
</Columns>
...
| `/pages/ver-mantenimientos.php` | Maintenance history |
